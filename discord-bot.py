...
AUTOROLE = #id roles for beginners

ROLES = {
    '🥃':    XXXXXXXXXXXXXXXXXX,  #ID of the roles that will be issued after adding the reaction to the post
    '🎮':    XXXXXXXXXXXXXXXXXX,  #ID of the roles that will be issued after adding the reaction to the post
}

ROLETYPE = {
    '🎮':    'NAME1',       #Names of roles that will be issued after adding the reaction to the post
    '🥃':    'NAME2',   #Names of roles that will be issued after adding the reaction to the post
}

RPG = {
    '1':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '2':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '3':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '4':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '5':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '6':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '7':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
    '8':    XXXXXXXXXXXXXXXXXX, #Role ID for the rpg system
}
...
system('cls')
bot = discord.Client()  # creation bot-client
...
async def on_message(message):   #async func for check - messeges
    ...
    if message.content == '!start':
        ...
        x = cur.fetchall()
        if x == []:
            cur.execute(f"INSERT INTO rpg (guild_id, member, member_name, member_dis, member_id, role_id, exp, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    if message.content == '!rank':
        ...
        x = cur.fetchall()
        if x == []:
            await channel.send('```RPG-система не подключена для вашего канала.\nНапишите <!start>, чтобы подключиться к ней```')
        else:
            for i in range(0, len(x)):
                x[i] = str(x[i])
                replacer(x, i)
